...
Run **slotalk init** in the project's root directory. This will parse the root directory's different source files for `@sloth` annotations and
...
Another way to parse a source file is to pass the input file through pipe to **slotalk**.
